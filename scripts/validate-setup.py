...
def check_environment_variables() -> Dict[str, bool]:
    ...
    # Check API keys
    if VOYAGE_API_KEY:
        print_status("Voyage API Key", True, "Found")
        results["voyage_key"] = True
    elif OPENAI_API_KEY:
        print_status("OpenAI API Key", True, "Found (Voyage not set)")
        results["openai_key"] = True
    else:
        print_status("API Keys", False, "No embedding API key found")
...
def check_qdrant_connection() -> Dict[str, bool]:
    ...
    try:
        ...
        # Check collections
        voyage_collections = [c for c in collections.collections if c.name.endswith("_voyage")]
        if voyage_collections:
            ...
            print_status("Collections", True, 
                        f"{len(voyage_collections)} collections, {total_vectors:,} vectors total")
    ...
    if VOYAGE_API_KEY:
        try:
            ...
            if response.status_code == 200:
                print_status("Voyage AI API", True, "Connected and authenticated")
                results["voyage_api"] = True
            else:
                print_status("Voyage AI API", False, f"Error {response.status_code}: {response.text}")
                results["voyage_api"] = False
                
        except Exception as e:
            print_status("Voyage AI API", False, f"Connection failed: {e}")
            results["voyage_api"] = False
...
def generate_report(all_results: Dict[str, Dict[str, bool]]) -> Tuple[int, int, int]:
    ...
    if not all_results.get("environment", {}).get("api_key", True):
        print(f"  1. Set up an API key:")
        print(f"     export VOYAGE_KEY='your-voyage-api-key'")
        print(f"     # or")
    ...
    if not all_results.get("qdrant", {}).get("collections", True):
        ...
        print(f"     python scripts/import-openai-enhanced.py")
